...
const Feedbacks = ({ menu }) => {
  ...
  return (
    <div>
      <h1 className="text-center text-white font-bold mt-[30px] text-[28px] lg:text-[35px] 2xl:text-[40px] ">
      ...
      <div className="w-full p-[25px] mt-[20px] 4xl:w-[1400px]   ">
        {isLoading ? (
          ...
          feedbacks.map((feedback, index) => (
            <div key={`${feedback.id}-${index}`} className="mb-[25px]">
              {!menu && (
                <>
                  <Fade left>
              ...
              {user && (
                <>
                  {!menu && (
                    <Fade left>
                      <button
                        className="mb-[30px] border-[1.5px] border-white  p-[5px] rounded-[10px] text-white xl:w-[80px]"
                        onClick={() => {
        ...
        {isLoading ? (
          ...
          <button
            ...
            className="w-full sm:w-[500px] text-white border-[2px] p-[8px] rounded-[10px] font-bold text-[20px] "
          >
            Leave a feedback
          </button>
        )}
